...
medialib.FolderItems["services/README.md"] = "Service extensions are services for more obscure websites. \
\
To use them, place them in one of the following folders:\
\
Folder | When to use\
---|---\
```garrysmod/addons/medialib/lua/medialib/services/``` | if you use medialib as an addon\
```garrysmod/addons/[youraddon]]/lua/medialib/services/``` | if you use ```dist/medialib.lua``` as part of your addon (```medialib/services/``` folders need to be created)\
```garrysmod/lua/medialib/services/``` | if you use ```dist/medialib.lua``` as part of your gamemode (```medialib/services/``` folders need to be created)\
\
If correctly installed, the service will automatically load."
-- Module media
...
\9\"https?://www.vimeo.com/channels/staffpicks/([0-9]+)\",\
\9\"https?://vimeo.com/channels/staffpicks/([0-9]+)\",\
}\
\
...
\9local h = str:match(\"(%d+)H\") or 0\
\9local m = str:match(\"(%d+)M\") or 0\
\9local s = str:match(\"(%d+)S\") or 0\
\9return h*(60*60) + m*60 + s\
end\
\
local API_KEY = \"AIzaSyBmQHvMSiOTrmBKJ0FFJ2LmNtc4YHyUJaQ\"\
function YoutubeService:query(url, callback)\
\9local urlData = self:parseUrl(url)\
\9local metaurl = string.format(\"https://www.googleapis.com/youtube/v3/videos?part=snippet%%2CcontentDetails&id=%s&key=%s\", urlData.id, API_KEY)\
\
\9http.Fetch(metaurl, function(result, size)\
\9\9if size == 0 then\
\9\9\9callback(\"http body size = 0\")\
...
\9\9local jsontbl = util.JSONToTable(result)\
\
\9\9if jsontbl and jsontbl.items then\
\9\9\9local item = jsontbl.items[1]\
\9\9\9if not item then\
\9\9\9\9callback(\"No video id found\")\
\9\9\9\9return\
\9\9\9end\
\
\9\9\9data.title = item.snippet.title\
\9\9\9data.duration = tonumber(PTToSeconds(item.contentDetails.duration))\
\9\9else\
